export const STABLE_ORDER_INDICES_U32_STATUS = Object.freeze({
  OK: 0,
  INVALID_EXTENT: 1,
...
function resetOrderingStatus(status) {
  ...
  if (i === gpu.u32(0)) status[gpu.u32(0)] = gpu.u32(0);
}
...
function stableOrderIndicesByKeyU32(keys, indicesIn, indicesOut, activeCount, keyCapacity, inputCapacity, outputCapacity, status) {
  ...
  if (active > outputCapacity) {
    if (i === gpu.u32(0)) gpu.atomic.cas(status, gpu.u32(0), gpu.u32(0), gpu.u32(2));
    return;
  }
  ...
  while (j < active) {
    ...
    if (otherKey < ownKey || (otherKey === ownKey && j < i)) rank++;
    j++;
  }
...
export const stableOrderIndicesU32DeviceProgram = Object.freeze({
  source,
  compile: Object.freeze({ headerProfile: 'cuda-cccl' }),
  functions: Object.freeze([
    Object.freeze({
      name: 'resetOrderingStatus', kind: 'kernel', returns: 'void',
      parameters: Object.freeze([Object.freeze({ name: 'status', type: 'ptr<u32>' })]),
    }),
    Object.freeze({
      name: 'stableOrderIndicesByKeyU32', kind: 'kernel', returns: 'void',
