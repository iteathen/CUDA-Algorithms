...
import { createExperiment, shape } from './plan.mjs';
import { reference } from './reference.mjs';

const mode = process.argv[2] ?? 'portable'; assert(['native', 'portable'].includes(mode));
const native = mode === 'native';
const maximum = shape(262144);
const options = { compiler: true, driver: { memory: { maxDeviceBytes: maximum.upperBoundBytes,
...
const results = []; const samples = []; const setups = []; const errors = []; let activePlan;
...
try {
  ...
  const fixture = JSON.parse(readFileSync(new URL('./fixtures/oqs-cut-five.json', import.meta.url)));
...
console.log(JSON.stringify({ outcome: native ? 'native-segment-scan-pass' : 'portable-segment-scan-submit-pass',
  nativeVerified: native, productionApi: false, gpuEqualityOrOrdering: false, payloadCopy: false,
  maximumCapacity: 262144, maximumDeviceBytes: maximum.deviceBytes, upperBoundBytes: maximum.upperBoundBytes,
  results, samples, setups, cleanup: 'graceful' }, null, 2));
